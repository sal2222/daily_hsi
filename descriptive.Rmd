...
to_scatter <-
  ...
  mutate_if(is.character, as.factor) %>% 
            mutate(age = fct_relevel(age, ">=40", after = Inf)) %>% 
   rename("Max Temp" = tmp_f_max,
         "Max HI" = heat_index_max,
         "Max WBGT" = wbgt_max,
         "Mean Temp" = tmp_f_mean,
         "Mean HI" = heat_index_mean,
         "Mean WBGT" = wbgt_mean,
         "Region" = region) 
...
to_scatter %>% 
    filter(case %in% "Case",
           `Max WBGT` > 85) %>% 
  dplyr::select(`Mean Temp`, `Mean HI`, `Mean WBGT`, Region) %>%
  ggplot(aes(x = `Mean Temp`, y = `Mean HI`, color = Region)) +
...
to_scatter %>% 
    filter(case %in% "Case",
                   `Max WBGT` > 85) %>% 
  dplyr::select(`Mean Temp`, `Mean HI`, `Mean WBGT`, Region) %>%
  ggplot(aes(x = `Mean Temp`, y = `Mean WBGT`, color = Region)) +
    geom_point(alpha = alpha_val) +
  #  geom_smooth(method = "lm", se = FALSE) +
     theme_bw()
...
to_scatter %>% 
    filter(case %in% "Case",
                   `Max WBGT` > 85) %>%  
  dplyr::select(`Mean Temp`, `Mean HI`, `Mean WBGT`, Region) %>%
  ggplot(aes(x = `Mean HI`, y = `Mean WBGT`, color = Region)) +
    geom_point(alpha = alpha_val) +
   # geom_smooth(method = "lm", se = FALSE) +
     theme_bw()
...
to_scatter %>% 
    filter(case %in% "Case") %>% 
  dplyr::select(`Max Temp`, `Max HI`, `Max WBGT`, Region) %>%
  filter(`Max WBGT` > 85) %>%
  ggplot(aes(x = `Max Temp`, y = `Max HI`, color = Region)) +
    geom_point(alpha = alpha_val) +
  #  geom_smooth(method = "lm", se = FALSE) +
     theme_bw()
...
to_scatter %>% 
  ...
  dplyr::select(`Max Temp`, `Max HI`, `Max WBGT`, Region) %>%
  filter(`Max WBGT` > 85) %>%
  ggplot(aes(x = `Max Temp`, y = `Max WBGT`, color = Region)) +
    geom_point(alpha = alpha_val) +
  #  geom_smooth(method = "lm", se = FALSE) +
     theme_bw()
...
to_scatter %>% 
  ...
  dplyr::select(`Max Temp`, `Max HI`, `Max WBGT`, Region) %>%
  filter(`Max WBGT` > 85) %>%
  ggplot(aes(x = `Max HI`, y = `Max WBGT`, color = Region)) +
    geom_point(alpha = alpha_val) +
   # geom_smooth(method = "lm", se = FALSE) +
     theme_bw()
