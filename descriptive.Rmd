...
to_scatter_cutoff <-
  ...
  #to_scatter (n= 139,875)
  #to_scatter_cutoff (Cases, n = 12,865)
...
#ggsave(filename = "output/plot_scatter_dual.tiff", device = "tiff", dpi = 700)
#ggsave("output/plot_scatter_dual.tiff)
...
ggsave(filename = "output/plot_index_pairs_scatter.tiff", device = "tiff", dpi = 700)
#ggsave("output/plot_index_pairs_scatter.png")

plot_index_pairs_scatter_2x3 <-
  (s1 | s2 ) / (s3 | s4 ) / ( s5 | s6) + 
    plot_layout(guides = "collect") + 
    plot_annotation(tag_levels = 'A') & 
    theme(legend.position = "bottom")
...
plot_index_pairs_scatter_2x3
#ggsave("output/plot_index_pairs_scatter_2x3.png")
...
Scatterplots with black points instead of colored by region
```{r}
...
to_scatter_cutoff %>% 
  dplyr::select(`Mean Temperature`, `Mean Heat Index`, `Mean WBGT`, Region) %>%
  ggplot(aes(x = `Mean Temperature`, y = `Mean Heat Index`)) +
    geom_point(alpha = alpha_val, size = point_size) +
    geom_smooth(method = "lm", se = FALSE, color = "black") +
    stat_ellipse(type = ellipse_type, size = ellipse_size, aes(color = Region)) +
    annotate(x = 90, y = 70, 
         label=paste("r = ", round(cor(to_scatter_cutoff$`Mean Temperature`, to_scatter_cutoff$`Mean Heat Index`),2)), 
...
Scatterplot all points (no temp cutoff)


```{r}

  #to_scatter (n= 139,875)
  #to_scatter_cutoff (n = 12,865)
...
plot_index_pairs_scatter_full_cases <-
  (s1 | s2 | s3) / (s4 | s5 | s6) + 
    plot_layout(guides = "collect") + 
    plot_annotation(tag_levels = 'A') & 
    theme(legend.position = "bottom")
...
ggsave(filename = "output/plot_index_pairs_scatter_full_cases.tiff", device = "tiff", dpi = 700)
#ggsave("output/plot_index_pairs_scatter.png")
...
ggsave("output/plot_density_dual_scale.tiff", width = 9.5, height = 4)
...
#mapping_df <-                              
 # read_rds(file = "data/mapping_df.rds")
...
## Export files as .shp for QGIS
```{r}

st_write(clim_region_union, "data/to_gis/clim_region_union.shp")

st_write(states_clim, "data/to_gis/states_clim.shp")

st_write(sites, "data/to_gis/sites.shp")
```
